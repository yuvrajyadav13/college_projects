...
#import quandl *** INCASE YOU WANT TO USE QUANDL UNCOMMENT LINE: 33,29 AND COMMENT LINE: 34
import pandas as pd
import numpy as np
import fbprophet
#import pytrends
...
class Stocker():
    ...
    def __init__(self, ticker, alpha_data, exchange='WIKI'):
        ...
        # quandl.ApiConfig.api_key = 'xVuFAe73budn7yyZgt-N'
        ...
        self.min_date = pd.to_datetime(min(stock['Date']))
        self.max_date = pd.to_datetime(max(stock['Date']))
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        m = train[x : ]
        del m['Adj. Close']
        train2 = pd.concat([n,m], sort=True)
        '''
    ...
    def retrieve_google_trends(self, search, date_range):
        ...
        #acc = pyGTrends("thehuedblog@gmail.com", "Yuvraj13@")
